...
function resolvePath(db: ProjectDatabase, projectId: string, filePath: string): string {
...
function listFilesRecursive(dir: string, extensions?: string[]): LocalFileInfo[] {
...
export function createFileOperationsHandlers(db: ProjectDatabase) {
  return {
    pm_read_file: async (input: { project: string; path: string; offset?: number; length?: number }) => {
      try {
    ...
    pm_write_file: async (input: { project: string; path: string; content: string; mode?: 'rewrite' | 'append' }) => {
      try {
        const fullPath = resolvePath(db, input.project, input.path);
    ...
    pm_search_files: async (input: { project: string; pattern: string; contentSearch?: string; maxResults?: number }) => {
      ...
      if (input.contentSearch) {
        matches = matches.filter(f => {
          try { return readFileSync(f.path, 'utf-8').includes(input.contentSearch!); } catch { return false; }
        });
      }
    ...
    pm_list_files: async (input: { project: string; path?: string; recursive?: boolean; extensions?: string[] }) => {
      const project = db.getProject(input.project);
      if (!project) return { error: `Project not found: ${input.project}` };
      ...
      if (input.recursive) {
        ...
        } catch { return { error: `Cannot access: ${targetPath}` }; }
      }
    ...
    pm_batch_read: async (input: { project: string; paths: string[] }) => {
    ...
    pm_get_file_info: async (input: { project: string; path: string }) => {
      try {
        const fullPath = resolvePath(db, input.project, input.path);
        const stats = statSync(fullPath);
